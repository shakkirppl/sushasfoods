   ...
   <body>
      ...
      <section>
        ...
        <div class="card mb-3 p-3 border">
            ...
            <h5 class="card-title">   @if($address->deafult==1) <input type="radio" name="address" id="address1" value="{{$address->id}}" checked> @else <input type="radio" name="address" id="address1" value="{{$address->id}}" >@endif {{$address->first_name}} {{$address->last_name}}</h5>
            <p class="mb-1">{{$address->address}},{{$address->landmark}},{{$address->city}}, @foreach($address->states as $state)  {{$state->name}}  @endforeach,{{$address->pincode}}</p>
            <p class="mb-1">@foreach($address->countrys as $country)  {{$country->country_name}}  @endforeach ,Phone number: {{$address->phone_number}}</p>
        </div>
        @endforeach
        @endif
        <a href="{{ url('address.new') }}" class="btn btn-outline-secondary btn-sm">+ New Address</a>
